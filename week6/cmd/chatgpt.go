...
func generateAndDeployResource(client *utils.OpenAI, userInput string) (string, error) {
	yamlContent, err := client.SendMessage(
		"你现在是一个 K8s 资源生成器，根据用户输入生成 K8s YAML，注意除了 YAML 内容以外不要输出任何内容，此外不要把 YAML 放在 ``` 代码快里",
		userInput,
	)
	if err != nil {
	...
	_, err = clientGo.DynamicClient.Resource(mapping.Resource).Namespace(namespace).Create(
		context.TODO(),
		unstructuredObj,
...
func queryResource(namespace, resourceType string) (string, error) {
	...
	resourceList, err := clientGo.DynamicClient.Resource(gvr).Namespace(namespace).List(
		context.TODO(),
		metav1.ListOptions{},
	)
	if err != nil {
...
func deleteResource(namespace, resourceType, resourceName string) (string, error) {
	...
	if err = clientGo.DynamicClient.Resource(gvr).Namespace(namespace).Delete(
		context.TODO(),
		resourceName,
	...
	return fmt.Sprintf("Deleted %s: %s", resourceType, resourceName), nil
}
